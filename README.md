...
5. **MP4 Video, GIF & PNG Export:** Ability to capture the animated Ateles creature in motion as a high-quality **MP4 video** (or WebM fallback) via native `MediaRecorder` API, export as an animated `.gif` file, or save a high-resolution PNG snapshot.
